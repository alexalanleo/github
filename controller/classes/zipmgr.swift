...
//  controller
//
//  Minimal ZIP reader/extractor (deflate) for IPA extraction.
//
//  Based on the implementation used in rooootdev/lara.
//
...
extension Data {
    fileprivate func scan<T>(at offset: Int) -> T {
        self.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: offset, as: T.self) }
    }

    fileprivate var zipCRC32: UInt32 {
...
public struct ZipEntry: Hashable {
    ...
    fileprivate let compressedSize: UInt64
    fileprivate let uncompressedSize: UInt64
    fileprivate let lfhOffset: UInt64
}
...
public final class ZipArchive {
    ...
    public init(data: Data) throws {
        self.data = data
        self.entries = try parseCentralDirectory()
    }

    public func extract(_ entry: ZipEntry) throws -> Data {
        let lfhOff = Int(entry.lfhOffset)
        guard lfhOff + 30 <= data.count else {
            throw ZipError.corruptArchive("(zip) truncated local header")
        }

        let sig: UInt32 = data.scan(at: lfhOff)
        guard sig == Self.lfhSignature else {
            throw ZipError.corruptArchive("(zip) bad local header signature")
        }

        let nameLen: UInt16 = data.scan(at: lfhOff + 26)
        let extraLen: UInt16 = data.scan(at: lfhOff + 28)
        let dataOff = lfhOff + 30 + Int(nameLen) + Int(extraLen)

        guard dataOff >= 0, dataOff + Int(entry.compressedSize) <= data.count else {
            throw ZipError.corruptArchive("(zip) entry data out of range")
        }

        let compressed = data.subdata(in: dataOff..<(dataOff + Int(entry.compressedSize)))
        ...
        case 8:
            // deflate
            return try decompressDeflate(compressed, decompressedSize: Int(entry.uncompressedSize))
        default:
            throw ZipError.unsupported("(zip) unsupported method \(entry.method)")
        }
    }

    private func parseCentralDirectory() throws -> [ZipEntry] {
        let eocd = try locateEOCD()
        let eocdOff = eocd.offset

        guard eocdOff + 22 <= data.count else {
            throw ZipError.corruptArchive("(zip) eocd truncated")
        }

        let cdCount: UInt16 = data.scan(at: eocdOff + 10)
        let cdSize: UInt32 = data.scan(at: eocdOff + 12)
        let cdOff: UInt32 = data.scan(at: eocdOff + 16)

        let start = Int(cdOff)
        let end = start + Int(cdSize)
        guard start >= 0, end <= data.count else {
            throw ZipError.corruptArchive("(zip) central directory out of range")
        }

        var pos = start
        var result: [ZipEntry] = []
        result.reserveCapacity(Int(cdCount))

        while pos + 46 <= end {
            let sig: UInt32 = data.scan(at: pos)
            guard sig == Self.cdSignature else {
                throw ZipError.corruptArchive("(zip) bad central directory signature")
            }

            let flags: UInt16 = data.scan(at: pos + 8)
            let method: UInt16 = data.scan(at: pos + 10)
            let crc32: UInt32 = data.scan(at: pos + 16)
            let compSize32: UInt32 = data.scan(at: pos + 20)
            let uncompSize32: UInt32 = data.scan(at: pos + 24)
            let nameLen: UInt16 = data.scan(at: pos + 28)
            let extraLen: UInt16 = data.scan(at: pos + 30)
            let commentLen: UInt16 = data.scan(at: pos + 32)
            let lfhOff32: UInt32 = data.scan(at: pos + 42)

            let nameStart = pos + 46
            let nameEnd = nameStart + Int(nameLen)
            guard nameEnd <= end else { throw ZipError.corruptArchive("(zip) name out of range") }

            let nameData = data.subdata(in: nameStart..<nameEnd)
            let path = String(data: nameData, encoding: .utf8) ?? ""
            let isDir = path.hasSuffix("/")

            var compressedSize = UInt64(compSize32)
            var uncompressedSize = UInt64(uncompSize32)
            var lfhOffset = UInt64(lfhOff32)

            // Zip64 extra is ignored for now unless needed.
            if extraLen > 0 {
                let extraStart = nameEnd
                let extraEnd = extraStart + Int(extraLen)
                if extraEnd <= end {
                    let extra = data.subdata(in: extraStart..<extraEnd)
                    let z64 = parseZIP64Extra(extra)
                    if let u = z64.uncompressedSize { uncompressedSize = u }
                    if let c = z64.compressedSize { compressedSize = c }
                    if let o = z64.relativeOffset { lfhOffset = o }
                }
            }
    ...
    private func decompressDeflate(_ compressed: Data, decompressedSize: Int) throws -> Data {
        ...
        guard status == Z_STREAM_END else {
            throw ZipError.corruptArchive("(zip) deflate failed with zlib status \(status)")
        }

        return Data(result.prefix(max(0, actualSize)))
    }
    ...
    private func locateEOCD() throws -> (offset: Int, commentLen: Int) {
        ...
        for i in (searchStart..<data.count - 3).reversed() {
            ...
            if sig == Self.eocdSignature {
                let commentLen: UInt16 = data.scan(at: i + 20)
                return (i, Int(commentLen))
            }
        }
        throw ZipError.corruptArchive("(zip) eocd not found")
    }
